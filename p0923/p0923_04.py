...
# with open("여기어때.html","w",encoding="utf-8") as f:
#     f.write(soup.prettify())

# 파일 불러오기
with open("여기어때.html","r",encoding="utf-8") as f:
    soup=BeautifulSoup(f,'lxml')


# 1.이미지, 숙소명,별점,리뷰수,금액 출력
...
for i in range(len(lis)):
    ...
    try:
        ...
        print("별점:",float(star))
        # 리뷰수
        reviwe=lis[i].find("span",{"class":"css-144z61f"}).get_text(strip=True)
        print("리뷰수:",int(reviwe[:-4].replace(",","")))
        # 금액
        money=lis[i].find("span",{"class":"css-1llao6q"}).get_text(strip=True)
        print("금액:",int(money.replace(",","")))
        print("-"*50)
